...
function getCoordinates() {
  ...
  useEffect(() => {
    fetch('https://api.opencagedata.com/geocode/v1/json?q=london&key=52535ae64e3048c58091a5065a58f57e')
      .then((resp) => resp.json())
      .then((data) => {
...
const SearchInput = () => {
  ...
  return <>
    <section className="search">
      <input placeholder='...' onKeyDown={event => checkEnter(event)} onChange={event => changeInput(event.target.value)}></input>
    </section>
  </>
}
